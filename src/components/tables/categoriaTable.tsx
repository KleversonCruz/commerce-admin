...
import { PencilIcon, PlusIcon, TagIcon, TrashIcon } from "@heroicons/react/outline";
import { useState } from "react";
import AlertModal from "@components/overlays/alertModal";

interface TableProps {
  categorias: Categoria[]
  selectAction: (categorias: Categoria) => void
  deleteAction: (categorias: Categoria) => void
...
export default function Table(props: TableProps) {
  ...
  const [selectedItem, setSelectectItem] = useState<Categoria>()
  const [currentPage, setCurrentPage] = useState(1)
  const [entriesPerPage] = useState(10)

  const indexOfLast = currentPage * entriesPerPage
  const indexOfFirst = indexOfLast - entriesPerPage
  const currentEntries = props.categorias.slice(indexOfFirst, indexOfLast)
  ...
  return (
    ...
    ">
        ...
        <div className="px-4 py-4 sm:px-6">
          <Pagination entriesPerPage={entriesPerPage} totalEntries={props.categorias.length} onPageChange={onPageChange} currentPage={currentPage} />
        </div>
      </div>
